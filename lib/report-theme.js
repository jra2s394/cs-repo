 ...
 *  header: string[]
 *  rows: (string | {text,fill?,color?,bold?})[][]  (defaults to [] — see note below)
 *  align: AlignmentType[] (optional, per column)
 *
 *  Defensive default for `rows`: if a caller passes `rows: undefined` (e.g.
 *  the underlying JSON omitted an optional field that isn't in REQUIRED),
 *  render an empty body instead of crashing inside `rows.map(...)`. The
 *  previous failure mode was a stack trace pointing at lib/report-theme.js
 *  with no hint about which report or field caused it. Per-report sites
 *  still SHOULD use `|| []` at the call so the intent stays explicit. */
...
module.exports = {
  ...
  run, para, gap, pageBreak,
  titleBanner, metaStrip, coverBlock, sectionHead, subHead,
