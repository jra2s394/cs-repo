...
function kpiStrip(cards) {
  ...
  const baseCardW = Math.floor((CW - gw * (n - 1)) / n);
  // Absorb any DXA remainder from integer truncation into the last card so
  // the widths array always sums to exactly CW (required by the docx layout engine).
  const remainder = CW - gw * (n - 1) - baseCardW * n;
  const widths = [], cells = [];
  cards.forEach((c, i) => {
    const cardW = i === n - 1 ? baseCardW + remainder : baseCardW;
    widths.push(cardW);
    ...
    if (i < n - 1) { widths.push(gw); cells.push(_plainCell(gw, null)); }
  });
...
module.exports = {
  ...
  titleBanner, metaStrip, sectionHead, subHead, caption,
  kpiStrip, dataTable, callout, recBlock, codeBlock,
  chartImage, centeredImage,
  buildDocument, render,
};
