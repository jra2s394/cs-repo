...
1. **Identity correlation.** Some commits are signed with
   `tripp.arnold@protonmail.com` and others with `tarnold@sysdynetechnologies.com`.
   Both addresses are visible in `git log --pretty=fuller`, which permanently
   links the two identities together.
2. **Typo in a commit message.** PR #23 landed with `prontonmail` in the
...
| Commit messages | Literal substring rewrites: `prontonmail` → `protonmail`, plus every customer/contact name replaced with a `[PLACEHOLDER]` token. |
...
git log --all -p -S 'Cemstone'
git log --all -p -S 'tarnold@sysdynetechnologies.com'
git log --all -p -S 'prontonmail'
git log --all -p -S '1204402035133218'
git log --all -p -S '1FGb57p0iKuJ1RhRGwrRez3q3RIUvF-bI'
```
