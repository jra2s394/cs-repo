...
### 🔴 Respond today (3)
1. Joe @ [CUSTOMER_G] · "Re: drive time averages" — asking how to compute avg drive-to-plant for the cubic-yards spreadsheet. Last reply: Anthony Sat 5/24 morning
   → Draft a quick reply explaining the dispatch calc.
2. [TEAMMATE_A] · "Re: Kurt Day 1 agenda" — wants confirmation Intercom invite is sent before Tue 5/26
   → Confirm yes (already sent Fri).
3. [CONTACT_A] · "[CUSTOMER_D] integration sync" — proposes Tue 1pm MT for a follow-up
   → Accept or counter-propose.

### 🟣 Escalate (1)
1. [CUSTOMER_E] · "Slabstack quote field showing wrong total" — bug, screenshot attached
   → Suggested: `/escalate <thread-url>` to file a Shortcut ticket for the quote-engine team

### 🟡 Customer signal (2)
1. [CUSTOMER_A] ([CONTACT_C]) — expansion signal: "we're spinning up a 4th plant in St Paul Q3" (body, 2nd paragraph)
   → Worth opening `/customer [CUSTOMER_A]` and noting in `/expansion`
2. [CUSTOMER_B] ([CONTACT_D]) — competitor mention: "evaluating [COMPETITOR_X] for the East coast contracts"
...
2. File the [CUSTOMER_E] bug as a Shortcut ticket
3. Note the [CUSTOMER_A] expansion signal before it gets buried
