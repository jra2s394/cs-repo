...
install_convention_templates() {
  ...
  for tmpl_file in "${templates_src}"/*.md; do
    ...
    local dest
    dest="${templates_dest}/$(basename "$tmpl_file")"
...
install_obsidian_integration() {
  ...
  # Copy the hook and patch the fallback vault path using Python to avoid
  # sed delimiter collisions if the vault path contains special characters.
  if python3 - "$hook_src" "$hook_dest" "$vault_path" <<'PYEOF'
import sys
src, dest, vault = sys.argv[1], sys.argv[2], sys.argv[3]
text = open(src, encoding="utf-8").read()
text = text.replace("/path/to/your/obsidian/vault", vault)
