...
Battle-tested `/health-score` against the real portfolio (one full Cemstone deep-dive + one portfolio scorecard across 30 active customers). Three live gaps surfaced that would otherwise show up as broken-looking commands for the next teammate. All three are now documented and structurally addressed in the skill prompt:

- **Single-customer mode added.** `/health-score <name>` now produces a focused per-customer snapshot (same rubric, one account, inline only — no `.docx`). Mode detection happens at Step 0; the portfolio path is unchanged. Useful before a customer call, a renewal, or any "is this account healthy?" check. The Cemstone deep-dive proved the value; without this, teammates would get a portfolio report when they wanted a focused one.
- **Asana team filter now in the prompt.** Without it, an unfiltered `get_projects` returns projects from sibling product teams (e.g., CEP CG & IS) that pollute the scorecard. Skill now instructs to pass the team GID; Slabstack's GID `1204402035133218` is documented and the discovery method (grep a known project URL) is spelled out for other teams.
