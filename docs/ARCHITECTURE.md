...
Hooks run at six event points during a session. The full list is in [CLAUDE.md § Active hooks](../CLAUDE.md#active-hooks); this section explains *when* each event fires relative to a tool call.
...
| `PostToolUse` | After a tool call returns | No — fire-and-forget (audit-log, notify) |
...
**The 9 commit-time blockers** (`branch-enforcer`, `push-guard`, `block-attribution`, `secret-scan`, `draft-before-create`, `file-protector`, `pr-template-reminder`, plus `audit-log` and `notify` running in PostToolUse) all hang off `PreToolUse` with `matcher: "Bash"` or a similar tool-name filter. The match string decides which hook fires for which tool — see `.claude/settings.json` for the live mapping.
