...
We currently wire 8 of the 29 hook events Claude Code supports (v2.1.141+). The full 29-event catalog with categories lives in [hooks/README.md § Available events](../hooks/README.md#available-events); this section explains *when each wired event fires* relative to a tool call, and which hooks use which.
...
**The commit-time blockers** (`branch-enforcer`, `push-guard`, `block-attribution`, `secret-scan`, `draft-before-create`, `file-protector`) all hang off `PreToolUse` with `matcher: "Bash"` or a similar tool-name filter. `pr-template-reminder` runs on `UserPromptSubmit`. `audit-log` + `notify` + `lint-after-edit` run on `PostToolUse` (audit-log also on `PostToolUseFailure`). `config-change-audit` runs on `ConfigChange`. `compact-reinject` on `PreCompact`. `session-to-obsidian` on `Stop`. The match string decides which hook fires for which tool — see `.claude/settings.json` for the live mapping.

The 21 events we don't wire are documented for completeness in hooks/README.md but aren't load-bearing today. Likely-next candidates if we want to extend: `SubagentStart` / `SubagentStop` (round-79 added a `code-reviewer` subagent — its lifecycle isn't logged yet), `PostCompact` (counterpart to our existing `PreCompact`), `WorktreeCreate` / `WorktreeRemove` (only relevant if we start using worktrees regularly).
