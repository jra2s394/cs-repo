"use strict";
// reports/onboarding-quarterly.js — Quarterly Onboarding CARR report generator
...
const outFile = path.join(outDir, `Onboarding_Quarterly_${yr}-${ql}.docx`);
...
if (d.monthlyTable && d.monthlyTable.length > 0) {
  // columns: Month | Started | Completed | CARR Completed | CARR In-Flight
  ...
  children.push(T.dataTable({
    ...
    header: ["Month", "Started", "Completed", "CARR Completed", "CARR In-Flight"],
...
// Account Detail (optional — accounts active this quarter)
if (d.accountsTable && d.accountsTable.length > 0) {
  // columns: Customer | Project | CARR | Started | Completed | Status
  // widths:  [2000, 1760, 1200, 1200, 1400, 1800] = 9360
  children.push(...T.sectionHead(`Account Detail — ${d.period}`));
  children.push(T.dataTable({
    columnWidths: [2000, 1760, 1200, 1200, 1400, 1800],
    header: ["Customer", "Project", "CARR", "Started", "Completed", "Status"],
...
children.push(T.codeBlock([
  `# Onboarding Quarterly Report — ${d.period}`,
...
T.render(doc, outFile)
  .then(() => { console.log(`✓ ${outFile}`); copyToDesktop(outFile, "Onboarding", "Quarterly"); })
